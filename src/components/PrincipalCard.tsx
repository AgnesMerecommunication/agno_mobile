    ...
    const go = async () => {
      ...
      if (principale) {
        ...
        if (cards.length > 0) {
          //@ts-ignore
          var item =
            cards.filter((item: {principale: number}) => item.principale == 1)
              .length > 0
              ? cards.filter(
                  (item: {principale: number}) => item.principale == 1,
                )[0]
              : cards[0];
          await downloadAndStoreImage(item.picture);
          let imageItem = await retrieveImage();
          ...
          var cardNumber = ModelCarte.filter(
            itemA => itemA.id == item.modelId,
          )[0];
    ...
    try {
      ...
      if (typeof userId === 'string') {
        await Share.open({
          ...
          title: 'Partager ma carte de visite',
        });
      } else {
          ...
          <View
            ...
            <ChangerCard
              ...
              onChange={(itema: any) => {
                setItem(itema);
                var cardNumber = ModelCarte.filter(
                  itemA => itemA.id == itema.modelId,
                )[0];
                const formData = new FormData();
                formData.append('principale', 1);
                dispatch(modifyOneBusinessCard(itema.id, formData));
                setHorizontal(cardNumber.horizontal);
                const formDataItem = new FormData();
                formDataItem.append('principale', 0);
                dispatch(modifyOneBusinessCard(item.id, formDataItem));
              }}
            />
            <SharedCard
