...
const TemplateCarteListVertical = () => {
  const [DATA_CARTE] = useState(ModelCarte.filter((item=>item.horizontal == false)));
  ...
  const getRender = ({item, index}: any) => (
    <TouchableOpacity
      style={{
        width: '100%',
        marginTop: '5%', backgroundColor : 'white',borderRadius : 10
      }}
      onPress={() => handleTemplate(item, index)}>
      <View style={{alignItems: 'flex-end', top: 20, zIndex: 1}}>
        {item.checked === true && (
          <CustomCircle
            disabled={true}
      ...
      <Image source={item.mod} style={{height: item.horizontal == true ? 200 : 600, width: '100%',borderRadius : 10}} />
    </TouchableOpacity>
  );
...
export default TemplateCarteListVertical;
